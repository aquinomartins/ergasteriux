...
function currentCsrfToken(){
  return window.__csrfToken || currentSession.csrf_token || null;
}
...
function needLogin(options={}){
  ...
  document.getElementById('view').innerHTML = `<h1>Login necessário</h1><p>Use o botão "Entrar" no cabeçalho (ou registre um novo usuário).</p>`;
}
...
async function refreshAuthUI(){
  ...
  currentSession = {
    ...
    csrf_token: s && s.csrf_token ? String(s.csrf_token) : null
  };

  if (currentSession.csrf_token) {
    window.__csrfToken = currentSession.csrf_token;
  }
...
function initAuth(){
  ...
  document.getElementById('loginForm').addEventListener('submit', async (e)=>{
    ...
    if (res.ok) {
      ...
      if (payload.csrf_token) {
        window.__csrfToken = payload.csrf_token;
      }
  ...
  document.getElementById('logoutBtn').addEventListener('click', async ()=>{
    await fetch(AUTH('logout.php'), { method:'POST', credentials:'include' });
    await refreshAuthUI();
    closeAuthOverlay();
